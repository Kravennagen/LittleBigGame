...
public:
    ...
    bool moveGhost(const std::string& tileset, sf::Vector2u tileSize, int x, int y){
    ...
    bool load(const std::string& tileset, sf::Vector2u tileSize, const int tiles[31][28], unsigned int width, unsigned int height)
    {
        ...
        for (unsigned int j = 0; j < width;/* height;*/ ++j/*i = 0; i < width; ++i*/)
            for (unsigned int i = 0; i < height;/*width;*/ ++i/*j = 0; j < height; ++j*/)
            {
